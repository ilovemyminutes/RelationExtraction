...
def verbose(phase: str, eval: dict, loss: float):
    ...
    prc = eval['precision']
    rec = eval['recall']
    print(f"[{phase}] ACC: {acc:.4f} F1: {f1:.4f} PRC: {prc:.4f} REC: {rec:.4f} Loss: {loss:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained-type", type=str, default=PreTrainedType.BertMultiLingual)
